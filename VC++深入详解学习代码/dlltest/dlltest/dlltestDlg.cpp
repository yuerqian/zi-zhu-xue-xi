...
BEGIN_MESSAGE_MAP(CdlltestDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_SUBTRACE, &CdlltestDlg::OnBnClickedBtnSubtrace)
...
void CdlltestDlg::OnBnClickedBtnAdd()
{
	...
	// TODO:  在此添加控件通知处理程序代码
	/*HINSTANCE Hinst;
	Hinst = LoadLibrary(_T("dll.dll"));
	typedef Point (*ADDPROC)(int a, int b);
	ADDPROC Add = (ADDPROC)GetProcAddress(Hinst, "Point");
	if (Add == NULL)
	{
		MessageBox(_T("获取函数失败"));
		return;
	}
	CString str;
...
void CdlltestDlg::OnBnClickedBtnSubtrace()
{
	// TODO:  在此添加控件通知处理程序代码
	HINSTANCE Hinst;
	Hinst = LoadLibrary(_T("dll.dll"));
	typedef int (*ADDPROC)(int a, int b);
	ADDPROC sub = (ADDPROC)GetProcAddress(Hinst, MAKEINTRESOURCEA(3));
	if (sub == NULL)
	{
		MessageBox(_T("获取函数失败"));
		return;
